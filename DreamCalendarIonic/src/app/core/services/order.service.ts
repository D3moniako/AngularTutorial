...
*/
  ...
  getUserPlanners(userId: number): Product[] {
    ...
    orders.forEach((order) => {
      if (order.status === 'PAGATO' && order.downloadAvailable) {
...
*/
  ...
  updateStatus(
    ...
    if (order) {
      ...
      if (oldStatus !== 'PAGATO' && status === 'PAGATO') {
        ...
        if (order.downloadAvailable) {
          this.notificationCenterService.add({
            ...
            message:
              'I tuoi planner digitali dell’ordine #' +
              order.id +
              ' sono pronti.',
...
*/
  ...
  getPaidOrders(): Order[] {
    return this.orders.filter((order) => order.status === 'PAGATO');
  }
