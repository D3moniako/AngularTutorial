...
export class PlannerService {
  ...
  private products: Product[] = [
...

    {
      ...
      price: 9.99,
      category: 'Wellness',
      description: 'Planner dedicato al benessere personale e alla cura di sé.',
      badge: 'WELLNESS',
      rating: 5,
      favorite: false,
  ...
  private favorites: Product[] = this.loadFavorites();
  ...
  private saveProducts() {
    const adminProducts = this.getOnlyAdminProducts();
  ...
  private loadAdminProducts() {
    ...
    try {
      ...
      console.log('Errore caricamento admin products');
    }
  }
  ...
  private getOnlyAdminProducts(): Product[] {
    ...
    return this.products.filter((p) => !baseIds.includes(p.id));
  }
  ...
  private loadReviews() {
    const saved = localStorage.getItem(this.REVIEWS_KEY);
  ...
  deleteProduct(id: number) {
    ...
    if (baseIds.includes(id)) {
      console.log('Impossibile eliminare prodotto base');
