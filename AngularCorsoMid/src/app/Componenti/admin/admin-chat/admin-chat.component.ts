...
export class AdminChatComponent implements OnInit {
  ...
  ngOnInit() {
    this.chatService.messages$.subscribe((data: ChatMessage[]) => {
      this.messages = data;
  ...
  loadCustomers(messages: ChatMessage[]) {
    ...
    messages.forEach((m) => {
      if (m.userId && m.sender === 'USER') {
        const exists = this.customers.find((c) => c.userId === m.userId);
  ...
  getConversation(): ChatMessage[] {
    ...
    return this.messages.filter((m) => m.userId === this.selectedUserId);
  }
