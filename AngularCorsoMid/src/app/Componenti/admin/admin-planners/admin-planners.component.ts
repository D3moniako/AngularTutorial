...
export class AdminPlannersComponent implements OnInit {
  ...
  updateStats() {
    ...
    this.premiumCount = this.products.filter(
      (p) => p.badge === 'PREMIUM' || p.badge === 'NUOVO',
    ).length;

    this.averagePrice = this.products.length
  ...
  savePlanner() {
    if (
      ...
      this.showMessage('⚠️ Inserisci nome, prezzo e categoria');

      return;
    }

    if (this.editing) {
      this.plannerService.updateProduct(this.newPlanner);

      this.showMessage('✅ Planner modificato');
    } else {
      this.newPlanner.id = Date.now();

      this.plannerService.addPlanner(this.newPlanner);

      this.showMessage('🎉 Planner pubblicato');
    }
  ...
  duplicatePlanner(product: Product) {
    ...
    this.plannerService.addPlanner(copy);

    this.showMessage('📋 Planner duplicato');

    this.loadProducts();
  }

  deletePlanner(id: number) {
    const ok = confirm('Eliminare definitivamente questo planner?');
    ...
    this.showMessage('🗑 Planner eliminato');
