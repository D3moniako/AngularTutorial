...
export class AdminUploadComponent {
  ...
  onImageSelected(event: any) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      this.message = '⚠️ Immagine troppo grande. Massimo 5 MB';
    ...
    this.message = '🖼️ Immagine caricata';
  }
  ...
  onPdfSelected(event: any) {
    ...
    if (file.type !== 'application/pdf') {
      this.message = '⚠️ Seleziona solamente un file PDF';
    ...
    if (file.size > 8 * 1024 * 1024) {
      this.message = '⚠️ PDF troppo grande. Massimo 8 MB';
    ...
    reader.onload = () => {
      this.planner.downloadUrl = reader.result as string;

      this.message = '📄 PDF caricato correttamente: ' + file.name;
    };

    reader.onerror = () => {
      this.message = '❌ Errore durante la lettura del PDF';
  ...
  savePlanner() {
    if (
      ...
      this.message = '⚠️ Inserisci nome, prezzo, categoria e descrizione';

      return;
    }

    if (!this.planner.image) {
      this.message = '⚠️ Inserisci una copertina';

      return;
    }

    if (!this.planner.downloadUrl) {
      this.message = '⚠️ Inserisci il PDF del planner';
