...
*/
  ...
  filter: 'ALL' | 'PAGATO' | 'IN_ATTESA' | 'ANNULLATO' = 'ALL';
...
*/
  ...
  calculateStats() {
    ...
    this.downloadCount = this.orders
    ...
    this.availableDownloads = this.orders

      .filter((o) => o.downloadAvailable && o.status === 'PAGATO')
...
*/
  ...
  applyFilters() {
    ...
    if (this.filter !== 'ALL') {
      result = result.filter((order) => order.status === this.filter);
    }
...
*/
    ...
    alert('Download disponibile per ordine #' + order.id);
  }
